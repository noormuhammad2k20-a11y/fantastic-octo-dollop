...
@endphp
<div class="row pro-calculator-app {{ (in_array(($tool['category'] ?? ''), ['medical', 'geometry', 'kitchen']) || $isFractionTool) ? 'stacked-layout' : '' }} {{ (request()->segment(count(request()->segments())) === 'spin-the-wheel') ? 'wheel-active' : '' }} {{ (request()->segment(count(request()->segments())) === 'coin-flipper') ? 'coin-active' : '' }}" id="pro-calculator-container" data-config="{{ json_encode($tool['pro_config'] ?? $tool['config'] ?? []) }}">
    <!-- Left Column: Inputs -->
    <div class="col-lg-12 mb-4">
        <div class="{{ $isFractionTool ? 'roi-style-card' : 'calculator-card' }} {{ $isFractionTool ? 'bp-style-card' : '' }}">
            ...
            <div class="input-section-label"><i class="fas fa-sliders-h"></i> Configuration Parameters</div>
                ...
                @else
                    @foreach($inputs as $input)
                        @include('tools.partials.pro-input', ['input' => $input])
                    @endforeach
                @endif

                @if(count($advancedInputs) > 0)
            ...
            <div class="calculator-footer border-top bg-light-soft p-3">
                ...
                <div class="quick-examples-row">
                    @foreach($examples as $ex)
                        <button class="btn-example-chip" data-json="{{ json_encode($ex['values']) }}">
                            {!! $ex['label'] !!}
                        </button>
                    @endforeach
                </div>
    ...
    <div class="col-lg-12">
        <div class="result-panel">
            @if(request()->segment(count(request()->segments())) === 'spin-the-wheel')
            ...
            @elseif(request()->segment(count(request()->segments())) === 'coin-flipper')
            ...
            @else
                <div class="{{ $isFractionTool ? 'roi-output-card' : ($isFractionTool ? 'output-card-themed' : 'result-card-v2') }}" id="pro-results-container" style="{{ $isFractionTool ? '--tool-hue:150;--tool-color:#10b981;--tool-bg:rgba(16,185,129,0.06);' : '' }}">
                    <div id="pro-main-result-card" style="display: none;">
                        ...
                        <div id="pro-sub-stats" class="{{ $isFractionTool ? 'row g-3 mt-4' : 'result-sub-stats mt-4' }}">
                    ...
                    <div id="pro-extra-results" class="mt-5 border-top pt-4" style="display: none;">
                        ...
                        <div id="pro-steps-container" class="mb-4" style="display: none;">
                            <h5 class="fw-bold fs-6 text-uppercase text-muted mb-3 {{ $isFractionTool ? 'small letter-spacing-1' : '' }}">
                                <i class="fas fa-list-ol me-2 text-info"></i> Calculation Steps
                            </h5>
                            <div id="pro-steps-list" class="{{ $isFractionTool ? 'bg-white shadow-sm border p-4 rounded-3 small text-secondary' : 'bg-light rounded p-3 small font-monospace text-dark border' }} break-words overflow-x-auto"></div>
                        </div>
                        <div id="pro-insights-container" style="display: none;" class="{{ $isFractionTool ? 'mt-4 p-4 bg-white rounded-3 border shadow-sm' : '' }}">
                            <h5 class="fw-bold fs-6 text-uppercase text-muted mb-3 {{ $isFractionTool ? 'small letter-spacing-1' : '' }}">
                                <i class="fas fa-lightbulb me-2 text-warning"></i> {{ $isFractionTool ? 'Mathematical Analysis' : 'Key Insights' }}
                            </h5>
...
<script src="{{ asset('js/core-math-engine.js') }}?v={{ time() }}"></script>
<script src="{{ asset('js/pro-calculator-engine.js') }}?v={{ time() }}"></script>
@if(request()->segment(count(request()->segments())) === 'spin-the-wheel')
<script src="{{ asset('js/wheel-engine.js') }}?v={{ time() }}"></script>
<script src="https://cdn.jsdelivr.net/npm/canvas-confetti@1.6.0/dist/confetti.browser.min.js"></script>
@endif
@if(request()->segment(count(request()->segments())) === 'coin-flipper')
<script src="{{ asset('js/coin-engine.js') }}?v={{ time() }}"></script>
<script src="https://cdn.jsdelivr.net/npm/canvas-confetti@1.6.0/dist/confetti.browser.min.js"></script>
@endif
