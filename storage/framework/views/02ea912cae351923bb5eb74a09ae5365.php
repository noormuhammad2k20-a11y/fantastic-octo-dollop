        ...
        <div class="row g-3 align-items-end mb-4">
            <div class="col-md-3">
                ...
                <select id="contentType" class="form-select border-2">
                    ...
                    <option value="bio">Page Bio</option>
                    <option value="event">Event Description</option>
                    </select>
            </div>            <div class="col-md-3">
                <label class="form-label fw-bold small text-uppercase text-muted">Topic / Business</label>
                <input type="text" id="topic" class="form-control border-2" placeholder="e.g. Pizza restaurant, fitness coach">
            </div>            <div class="col-md-3">
                <label class="form-label fw-bold small text-uppercase text-muted">Audience</label>
                <select id="audience" class="form-select border-2">
            ...
            <div class="col-md-3">
                <button id="gen-btn" class="btn btn-accent px-4 py-2 w-100 fw-bold rounded-3">
                    <i class="fab fa-facebook me-2"></i> Generate
                </button>
            </div>
        </div>
        <div id="gen-results" class="d-none">
            <h5 class="fw-bold mb-3"><i class="fab fa-facebook me-2 text-primary"></i>Generated Content</h5>
            <div id="gen-list" class="list-group gap-2"></div>
        </div>
        <div id="gen-placeholder" class="text-center py-5">
            <div class="opacity-25 mb-3"><i class="fab fa-facebook fa-4x"></i></div>
            <h5 class="text-muted">Choose a content type to generate Facebook content</h5>
        </div>
    </div>
</div>
<style>
.btn-accent{background:linear-gradient(135deg,#1877f2,#42b72a);color:#fff;border:none;transition:.3s}
...
.list-group-item-action:hover{border-color:#1877f2!important;background:#fafafa}
.copy-icon{opacity:0;transition:.2s}
.list-group-item-action:hover .copy-icon{opacity:1}
</style>
<script>
document.addEventListener('DOMContentLoaded',function(){
const btn=document.getElementById('gen-btn'),contentType=document.getElementById('contentType'),topic=document.getElementById('topic'),audience=document.getElementById('audience'),results=document.getElementById('gen-results'),list=document.getElementById('gen-list'),ph=document.getElementById('gen-placeholder');
btn.addEventListener('click',function(){
    btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin me-2"></i>Generating...';
    fetch('<?php echo e(route("ai.generate",["type"=>"facebook-ai"])); ?>',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'},body:JSON.stringify({contentType:contentType.value,topic:topic.value,audience:audience.value})})
    .then(r=>r.json()).then(data=>{
        ...
        list.appendChild(d);});}
    }).finally(function(){btn.disabled=false;btn.innerHTML='<i class="fab fa-facebook me-2"></i>Generate';});
});
});
</script><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\facebook-ai-tools.blade.php ENDPATH**/ ?>