<div class="row g-4 stats-suite-rebuilt">
    <div class="col-lg-12">
        <div class="calculator-card">
            
            <div class="calculator-body">
                <div class="row g-4">
                    <div class="col-md-4">
                        <label class="form-label-custom">Raw Score (x)</label>
                        <input type="number" id="input-x" class="form-control form-control-lg" value="85">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label-custom">Population Mean (μ)</label>
                        <input type="number" id="input-mu" class="form-control form-control-lg" value="70">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label-custom">Std Deviation (σ)</label>
                        <input type="number" id="input-sigma" class="form-control form-control-lg" value="10">
                    </div>
                </div>
                <div class="row g-3 mt-2">
                    <div class="col-md-12 d-flex gap-2">
                        <button class="btn btn-primary-stats flex-grow-1 py-3 fw-bold" id="btn-calculate" style="min-width: 280px; max-width: 100%; background:#3b82f6;box-shadow:0 4px 12px rgba(59,130,246,0.2)">
                            <i class="fas fa-play me-2"></i>Calculate Z-Score
                        </button>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="col-lg-12">
        <div id="results-card" class="output-card-themed" style="--tool-hue:210;--tool-color:#3b82f6;--tool-bg:rgba(59,130,246,.04); display: none;">
            <div class="output-hero">
                <span class="output-hero-label">Standardized Score (Z)</span>
                <div class="output-hero-value" id="res-z">0</div>
                <span class="output-hero-unit" id="res-z-desc">Above the Mean</span>
            </div>

            <div class="row g-4 mt-4">
                <div class="col-md-6">
                    <div class="stat-pill">
                        <span class="label">Percentile (approx)</span>
                        <span class="value" id="res-percentile">0%</span>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="stat-pill">
                        <span class="label">Probability P(X < x)</span>
                        <span class="value" id="res-p">0</span>
                    </div>
                </div>
            </div>

            <div class="step-container mt-5">
                <h6 class="fw-bold mb-4"><i class="fas fa-list-ol me-2 text-primary"></i>How the Z-Score is Calculated</h6>
                <div class="step-card mb-3">
                    <div class="step-header">The Formula</div>
                    <div class="step-body">
                        <div class="formula-block text-center py-3">
                            <span class="fs-4">z = (x - μ) / σ</span><br>
                            <span class="fs-5 mt-2 d-inline-block" id="step-formula">z = (85 - 70) / 10 = 1.5</span>
                        </div>
                    </div>
                </div>
                <div class="step-card mb-3">
                    <div class="step-header">Interpretation</div>
                    <div class="step-body" id="step-interpretation">
                        Your score is 1.5 standard deviations above the population mean.
                    </div>
                </div>
            </div>

            <div class="d-flex flex-wrap gap-2 mt-4">
                <button class="btn btn-dark-stats px-4 py-3 rounded-3 flex-grow-1" id="btn-copy" style="min-width: 280px; max-width: 100%;">
                    <i class="fas fa-copy me-2"></i>Copy Score Report
                </button>
            </div>
        </div>
    </div>
</div>

<script src="https://cdnjs.cloudflare.com/ajax/libs/mathjs/11.8.0/math.js"></script>
<script>
document.addEventListener('DOMContentLoaded', function() {
    const $ = id => document.getElementById(id);
    const btnCalc = $('btn-calculate');
    const resultsCard = $('results-card');

    // Standard Normal CDF approximation (Abramowitz & Stegun)
    function normalCDF(z) {
        const t = 1 / (1 + 0.2316419 * Math.abs(z));
        const d = 0.3989423 * Math.exp(-z * z / 2);
        const prob = d * t * (0.3193815 + t * (-0.3565638 + t * (1.781478 + t * (-1.821256 + t * 1.330274))));
        return z > 0 ? 1 - prob : prob;
    }

    function calculate() {
        const x = parseFloat($('input-x').value);
        const mu = parseFloat($('input-mu').value);
        const sigma = parseFloat($('input-sigma').value);

        if (isNaN(x) || isNaN(mu) || isNaN(sigma) || sigma <= 0) {
            alert('Please enter valid numerical values (Sigma must be > 0)');
            return;
        }

        const z = (x - mu) / sigma;
        const p = normalCDF(z);
        const percentile = (p * 100).toFixed(2);

        $('res-z').textContent = z.toFixed(4);
        $('res-p').textContent = p.toFixed(5);
        $('res-percentile').textContent = percentile + "%";
        $('res-z-desc').textContent = z >= 0 ? "Above the Mean" : "Below the Mean";

        $('step-formula').textContent = `z = (${x} - ${mu}) / ${sigma} = ${z.toFixed(4)}`;
        $('step-interpretation').innerHTML = `A Z-Score of <strong>${z.toFixed(2)}</strong> means your data point is <strong>${Math.abs(z).toFixed(2)}</strong> standard deviations <strong>${z >= 0 ? 'above' : 'below'}</strong> the mean. Approximately <strong>${percentile}%</strong> of the population falls below this value.`;

        resultsCard.style.display = 'block';
        resultsCard.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }

    btnCalc.addEventListener('click', calculate);
    $('btn-copy').addEventListener('click', function() {
        const text = `Z-Score Report:\nZ-Score: ${$('res-z').textContent}\nPercentile: ${$('res-percentile').textContent}\nGenerated via ToolsHub`;
        navigator.clipboard.writeText(text).then(() => {
            const o = this.innerHTML; this.innerHTML = 'Copied!';
            setTimeout(() => this.innerHTML = o, 2000);
        });
    });
});
</script>

<style>
.stats-suite-rebuilt .calculator-card { background: #fff; border: 1px solid #e2e8f0; border-radius: 24px; padding: 2.5rem; box-shadow: 0 10px 30px rgba(0,0,0,0.03); }
.stats-suite-rebuilt .calculator-header { display: flex; align-items: center; gap: 1.2rem; margin-bottom: 2.5rem; }
.stats-suite-rebuilt .calculator-header h4 { margin: 0; font-weight: 800; color: #0f172a; }
.stats-suite-rebuilt .calculator-header p { margin: 0; font-size: 0.9rem; color: #64748b; }
.stats-suite-rebuilt .tool-icon-circle { width: 64px; height: 64px; border-radius: 18px; display: flex; align-items: center; justify-content: center; font-size: 1.6rem; flex-shrink: 0; }
.stats-suite-rebuilt .form-label-custom { font-size: 0.8rem; font-weight: 700; color: #475569; text-transform: uppercase; margin-bottom: 0.6rem; display: block; }
.btn-primary-stats { color: #fff; border: none; border-radius: 12px; transition: all 0.3s; }
.btn-dark-stats { background: #0f172a; color: #fff; border: none; border-radius: 12px; }
.output-card-themed { background: #fff; border: 2px solid #e2e8f0; border-radius: 24px; padding: 2.5rem; margin-top: 1rem; }
.output-hero { text-align: center; padding: 2rem; background: var(--tool-bg); border-radius: 20px; border: 1px solid rgba(0,0,0,0.05); }
.output-hero-label { font-size: 0.9rem; font-weight: 700; color: var(--tool-color); text-transform: uppercase; }
.output-hero-value { font-size: 4rem; font-weight: 900; color: #0f172a; margin: 0.5rem 0; }
.stat-pill { background: #f8fafc; padding: 1.2rem; border-radius: 16px; border: 1px solid #f1f5f9; display: flex; flex-direction: column; align-items: center; }
.stat-pill .label { font-size: 0.7rem; font-weight: 700; color: #64748b; text-transform: uppercase; }
.stat-pill .value { font-size: 1.4rem; font-weight: 800; color: #0f172a; }
.formula-block { background: #0f172a; color: #e2e8f0; padding: 1.5rem; border-radius: 12px; font-family: monospace; }
</style>

<?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\z-score-calculator.blade.php ENDPATH**/ ?>