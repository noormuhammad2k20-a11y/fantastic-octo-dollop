<div class="tool-interactive-container">
    <div class="card border-0 shadow-sm p-4 mb-4 bg-white rounded-4">
        <div class="row g-3 align-items-end mb-4">
            <div class="col-md-4">
                <label class="form-label fw-bold small text-uppercase text-muted">Essay Subject</label>
                <input type="text" id="topic" class="form-control border-2" placeholder="e.g. Climate Change Impact">
            </div>            <div class="col-md-4">
                <label class="form-label fw-bold small text-uppercase text-muted">Essay Type</label>
                <select id="essayType" class="form-select border-2">
            ...
            <div class="col-md-4">
                <button id="gen-btn" class="btn btn-accent px-4 py-2 w-100 fw-bold rounded-3">
                    <i class="fas fa-heading me-2"></i> Generate
                </button>
            </div>
        </div>
        <div id="gen-results" class="d-none">
            <h5 class="fw-bold mb-3"><i class="fas fa-heading me-2 text-primary"></i>Generated Essay Titles</h5>
            <div id="gen-list" class="list-group gap-2"></div>
        </div>
        <div id="gen-placeholder" class="text-center py-5">
            <div class="opacity-25 mb-3"><i class="fas fa-heading fa-4x"></i></div>
            <h5 class="text-muted">Enter your essay subject to generate titles</h5>
        </div>
    </div>
</div>
<style>
...
const btn=document.getElementById('gen-btn'),topic=document.getElementById('topic'),essayType=document.getElementById('essayType'),results=document.getElementById('gen-results'),list=document.getElementById('gen-list'),ph=document.getElementById('gen-placeholder');
btn.addEventListener('click',function(){
    btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin me-2"></i>Generating...';
    fetch('<?php echo e(route("ai.generate",["type"=>"essay-title"])); ?>',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'},body:JSON.stringify({topic:topic.value,essayType:essayType.value})})
    .then(r=>r.json()).then(data=>{
        ...
        list.appendChild(d);});}
    }).finally(function(){btn.disabled=false;btn.innerHTML='<i class="fas fa-heading me-2"></i>Generate';});
});
});
</script><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\essay-title-generator.blade.php ENDPATH**/ ?>