        ...
        <div class="card-body-v2 p-4">
            ...
            <div class="p-3 rounded-4 mb-4" style="background-color: #f8fafc; border: 1.5px solid #e2e8f0;">
                ...
                <div class="d-flex flex-wrap gap-2">
                    <button type="button" class="btn btn-outline-dark btn-sm rounded-pill px-3" id="btn-sample-even">Even Dataset</button>
                    <button type="button" class="btn btn-outline-dark btn-sm rounded-pill px-3" id="btn-sample-odd">Odd Dataset</button>
                </div>
            </div>
                    ...
                    <div class="p-4 rounded-4 h-100" style="background-color: #fff; border: 1.5px solid #f1f5f9;">
                        ...
                        <div class="row g-3">
                            <div class="col-md-9">
                                ...
                                <div class="invalid-feedback">Requires at least 3 valid numbers.</div>
                            </div>
                            <div class="col-md-3">
    ...
    <div id="result-card" class="card tool-card-stacked shadow-sm border-0 d-none">
        <div class="card-header-v2 bg-white border-bottom-0 py-4 px-4">
            <div class="d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h5 class="mb-0 fw-bold text-dark">Dispersion Analysis</h5>
                        <p class="text-muted small mb-0">Quartiles and coefficients</p>
                    </div>
                </div>
            </div>
        </div>
        <div class="card-body-v2 p-4">
            <div class="row g-3 mb-4">
                <div class="col-md-3">
                    <div class="p-3 rounded-4 bg-light border text-center h-100">
                        ...
                        <div class="h4 fw-bold mb-0 text-dark" id="out-q1">0</div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="p-3 rounded-4 bg-light border text-center h-100">
                        <div class="small fw-bold text-uppercase text-muted mb-1">Median (Q2)</div>
                        <div class="h4 fw-bold mb-0 text-dark" id="out-median">0</div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="p-3 rounded-4 bg-light border text-center h-100">
                        <div class="small fw-bold text-uppercase text-muted mb-1">Third Quartile (Q3)</div>
                        <div class="h4 fw-bold mb-0 text-dark" id="out-q3">0</div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="p-3 rounded-4 bg-light border text-center h-100">
                        <div class="small fw-bold text-uppercase text-muted mb-1">Interquartile Range</div>
            ...
            <div class="row g-4 mb-4">
                <div class="col-md-6">
                    <div class="p-4 rounded-4 bg-primary-soft border border-primary h-100 text-center">
                        <div class="small fw-bold text-uppercase text-primary mb-2">Quartile Deviation</div>
                        <div class="display-5 fw-bold mb-0 text-primary" id="out-qd">0</div>
                        <p class="text-muted small mt-2 mb-0">The semi-interquartile range</p>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="p-4 rounded-4 bg-success-soft border border-success h-100 text-center">
                        <div class="small fw-bold text-uppercase text-success mb-2">Coefficient of QD</div>
                        <div class="display-5 fw-bold mb-0 text-success" id="out-coeff">0</div>
                        <p class="text-muted small mt-2 mb-0">Relative measure of dispersion</p>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const outQd = document.getElementById('out-qd');
    const outCoeff = document.getElementById('out-coeff');
    ...
    function calculate() {
        ...
        if (rawData.length < 3 || rawData.some(isNaN)) {
        ...
        outQd.textContent = qd.toFixed(prec);
        outCoeff.textContent = coeff.toFixed(prec);

        // Math Update
        setTimeout(() => {
            renderMath('math-qd', `\\text{Quartile Deviation (QD)} = \\frac{Q_3 - Q_1}{2} = \\frac{${q3.toFixed(prec)} - ${q1.toFixed(prec)}}{2} = ${qd.toFixed(prec)}`);
            renderMath('math-coeff', `\\text{Coefficient of QD} = \\frac{Q_3 - Q_1}{Q_3 + Q_1} = \\frac{${q3.toFixed(prec)} - ${q1.toFixed(prec)}}{${q3.toFixed(prec)} + ${q1.toFixed(prec)}} = ${coeff.toFixed(prec)}`);
        }, 100);
    ...
    document.getElementById('btn-sample-even').addEventListener('click', () => {
        dataE.value = "10, 15, 20, 25, 30, 35, 40, 45";
        calculate();
    });
    document.getElementById('btn-sample-odd').addEventListener('click', () => {
        dataE.value = "5, 7, 8, 12, 14, 18, 20";
        calculate();
    });
...
<?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\quartile-deviation-calculator.blade.php ENDPATH**/ ?>