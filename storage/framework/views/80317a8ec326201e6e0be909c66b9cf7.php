            ...
            <div class="row g-4">
                <div class="col-md-8">
                    <div class="p-4 rounded-4 h-100" style="background-color: #fff; border: 1.5px solid #f1f5f9;">
                        <h6 class="fw-bold text-uppercase text-muted small mb-3 letter-spacing-1">Sphere Dimensions</h6>
                        <div class="row g-3">
                            <div class="col-12">
                                <label class="form-label small fw-bold text-secondary text-uppercase mb-2">Radius (r)</label>
                                <div class="input-group">
                                    <input type="number" id="in-radius" class="form-control form-control-lg rounded-start-3" value="5" step="any" min="0">
                                    <select id="in-unit" class="form-select form-select-lg rounded-end-3" style="max-width: 100px;">
        ...
        <div class="card-body-v2 p-4" style="overflow-x: auto;">
            <div class="row g-4 align-items-center mb-4">
                ...
                <div class="col-lg-6 text-center">
                    ...
                    <p class="text-muted fw-bold text-uppercase small letter-spacing-1 mt-2">Surface Area (<span class="out-unit-2"></span>)</p>
                </div>
            </div>
...
<style>
    ...
    .input-group .form-select { border-left: 1.5px solid var(--border-color); }
...
document.addEventListener('DOMContentLoaded', function() {
    const radiusE = document.getElementById('in-radius');
    ...
    function calculate() {
        const r = parseFloat(radiusE.value);
        const u = unitE.value;
        ...
        const surface = 4 * pi * Math.pow(r, 2);
        const diameter = 2 * r;
        const circumference = 2 * pi * r;
        
        // Update UI
        outVolume.textContent = volume.toFixed(p);
        outSurface.textContent = surface.toFixed(p);
        
        outUnit3.forEach(el => el.textContent = u + '³');
        outUnit2.forEach(el => el.textContent = u + '²');
        
        const ins = [
            `<strong>Diameter:</strong> ${diameter.toFixed(p)} ${u}`,
            `<strong>Circumference:</strong> ${circumference.toFixed(p)} ${u}`,
            `<strong>Surface to Volume Ratio:</strong> ${(surface/volume).toFixed(p)} ${u}⁻¹`,
    ...
    btnCopyFormula.addEventListener('click', function() {
        navigator.clipboard.writeText('V = (4/3) * π * r³').then(() => {
            const btn = this;
...
<?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\volume-sphere-calculator.blade.php ENDPATH**/ ?>