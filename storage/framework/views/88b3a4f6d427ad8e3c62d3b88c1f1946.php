        ...
        <div class="calculator-card">
            <div class="calculator-header d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center gap-2">
                    <div class="tool-icon-circle" style="background:rgba(234,88,12,.05);color:#ea580c"><i class="fas fa-hammer"></i></div>
                    <h5 class="mb-0 fw-bold">Decking Calculator</h5>
                </div>
            ...
            <div class="calculator-body mt-2">
                <div class="row g-2">
                    <div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Deck Length</label><input type='number' id='calc-deck-len' class='form-control form-control-sm rounded-2 shadow-none border-secondary-subtle py-1' value='20' step='1'></div><div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Deck Width</label><input type='number' id='calc-deck-width' class='form-control form-control-sm rounded-2 shadow-none border-secondary-subtle py-1' value='16' step='1'></div><div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Board Width (in)</label><input type='number' id='calc-board-width' class='form-control form-control-sm rounded-2 shadow-none border-secondary-subtle py-1' value='5.5' step='1'></div><div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Gap (in)</label><input type='number' id='calc-gap' class='form-control form-control-sm rounded-2 shadow-none border-secondary-subtle py-1' value='0.25' step='1'></div><div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Wastage</label><select id='calc-wastage' class='form-select form-select-sm rounded-2 shadow-none border-secondary-subtle py-1'><option value='0' >0%</option><option value='5' >5%</option><option value='10' selected>10%</option><option value='15' >15%</option></select></div>
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function renderFormula() {
        if (typeof katex !== 'undefined') {
            katex.render("N = \\frac{L_{deck}}{W_{board} + G} \\times \\frac{L_{board}}{L_{deck}}", $('latex-formula'), {throwOnError: false, displayMode: true});
        } else setTimeout(renderFormula, 100);
    }
    renderFormula();

        function calculate() {
        const l = parseFloat($('calc-deck-len').value)||0, w = parseFloat($('calc-deck-width').value)||0, wast = parseFloat($('calc-wastage').value)||0;
        const b = Math.ceil((l*w/0.48) / 16 * (1+wast/100));
        $('res-main').textContent = b;
        $('res-unit').textContent = '16ft Boards';
        $('res-details').innerHTML = `<div class="stat-box"><div class="small text-muted">Area</div><div class="fw-bold">${l*w} ft²</div></div>`;
    }
    $('btn-calc').addEventListener('click', calculate);
    ['calc-deck-len','calc-deck-width','calc-wastage'].forEach(id=>$(id).addEventListener('input',calculate));
    calculate();
});
</script>
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\decking-calculator.blade.php ENDPATH**/ ?>