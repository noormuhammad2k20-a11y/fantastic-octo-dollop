    ...
    <div class="card tool-card-stacked mb-4 shadow-sm border-0">
        ...
        <div class="card-body-v2 p-4">
                ...
                <div class="col-md-12">
                    <div class="p-4 rounded-4 h-100" style="background-color: #fff; border: 1.5px solid #f1f5f9;">
                        ...
                        <div class="row g-3 align-items-end">
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-secondary text-uppercase mb-2">Square Meters (sq m)</label>
                                <input type="number" id="input-val" class="form-control form-control-lg rounded-3" value="1000" step="any" min="0">
                            </div>
                            <div class="col-md-6">
                        ...
                        <div class="mt-4 d-flex flex-wrap gap-2">
                            <button type="button" class="btn btn-outline-dark btn-sm rounded-pill px-3 quick-val" data-val="1000">1000 sq m</button>
                            <button type="button" class="btn btn-outline-dark btn-sm rounded-pill px-3 quick-val" data-val="2000">2000 sq m</button>
                            <button type="button" class="btn btn-outline-dark btn-sm rounded-pill px-3 quick-val" data-val="4046">4046 sq m</button>
                            <button type="button" class="btn btn-outline-dark btn-sm rounded-pill px-3 quick-val" data-val="5000">5000 sq m</button>
                            <button type="button" class="btn btn-outline-dark btn-sm rounded-pill px-3 quick-val" data-val="10000">10000 sq m</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-4 d-flex flex-wrap justify-content-center gap-3">
                ...
                <a href="/acres-to-square-meters-converter" class="btn btn-outline-secondary btn-lg rounded-pill px-4 shadow-sm transition-all" title="Swap Units"><i class="fas fa-exchange-alt"></i></a>
            </div>
        </div>
    </div>
            ...
            <div class="row g-4 align-items-center mb-4">
                <div class="col-lg-12 text-center">
                    <div class="p-4 rounded-4 bg-light border">
                        <div class="display-4 fw-bold text-primary mb-2 text-break" id="out-result">0.0000</div>
                        <p class="text-muted fw-bold text-uppercase small letter-spacing-1 mb-0">Acres (ac)</p>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculate() {
        ...
        setTimeout(() => {
            let res = (function(val) { return (val / 4046.85642); })(val);
            ...
            let formulaLatex = "\\text{Acres} = \\frac{\\text{Square Meters}}{4046.85642}";
            let stepLatex = "\\text{Acres} = \\frac<?php echo e(val); ?>{4046.85642} = {res}\\text{ ac}".replace('{val}', formattedVal).replace('{res}', formattedRes);
            
            mathSteps.innerHTML = `
                <div class="mb-2 text-muted small">Base Formula:</div>
                <div class="text-center mb-3">\\\\[ ${formulaLatex} \\\\]</div>
                <div class="mb-2 text-muted small">Calculation:</div>
                <div class="text-center">\\\\[ ${stepLatex} \\\\]</div>
            `;
    ...
    btnCopy.addEventListener('click', function() {
        const text = `${inputE.value} sq m = ${outResult.textContent} ac\nGenerated via ToolsHub.`;
...
</script><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\square-meters-to-acres-converter.blade.php ENDPATH**/ ?>