        ...
        <div class="calculator-card">
            <div class="calculator-header d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center gap-2">
                    <div class="tool-icon-circle" style="background:rgba(234,88,12,.05);color:#ea580c"><i class="fas fa-snowflake"></i></div>
                    <h5 class="mb-0 fw-bold">AC BTU Calculator</h5>
                </div>
            ...
            <div class="calculator-body mt-2">
                <div class="row g-2">
                    <div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Area (ft²)</label><input type='number' id='calc-room-area' class='form-control form-control-sm rounded-2 shadow-none border-secondary-subtle py-1' value='250' step='1'></div><div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Ceiling (ft)</label><input type='number' id='calc-ceiling-ht' class='form-control form-control-sm rounded-2 shadow-none border-secondary-subtle py-1' value='8' step='1'></div><div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>People</label><input type='number' id='calc-occupants' class='form-control form-control-sm rounded-2 shadow-none border-secondary-subtle py-1' value='2' step='1'></div><div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Sun</label><select id='calc-exposure' class='form-select form-select-sm rounded-2 shadow-none border-secondary-subtle py-1'><option value='shaded' >Shaded</option><option value='normal' >Normal</option><option value='sunny' >Sunny</option></select></div><div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Kitchen?</label><select id='calc-kitchen' class='form-select form-select-sm rounded-2 shadow-none border-secondary-subtle py-1'><option value='no' >No</option><option value='yes' >Yes</option></select></div>
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function renderFormula() {
        if (typeof katex !== 'undefined') {
            katex.render("BTU = Area \\times 20 + Adjustments", $('latex-formula'), {throwOnError: false, displayMode: true});
        } else setTimeout(renderFormula, 100);
    }
    renderFormula();

        function calculate() {
        const a = parseFloat($('calc-room-area').value)||0, kit = $('calc-kitchen').value;
        let btu = a*20; if(kit==='yes') btu+=4000;
        $('res-main').textContent = btu.toLocaleString();
        $('res-unit').textContent = 'BTU';
        $('res-details').innerHTML = `<div class="stat-box"><div class="small text-muted">Tons</div><div class="fw-bold">${(btu/12000).toFixed(1)}</div></div>`;
    }
    $('btn-calc').addEventListener('click', calculate);
    ['calc-room-area','calc-kitchen'].forEach(id=>$(id).addEventListener('input',calculate));
    calculate();
});
</script>
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\ac-btu-calculator.blade.php ENDPATH**/ ?>