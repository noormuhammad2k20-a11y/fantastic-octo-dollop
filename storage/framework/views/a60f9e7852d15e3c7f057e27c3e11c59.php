            ...
            <div class="calculator-body">
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <input type="number" id="in-sides" class="form-control form-control-lg rounded-3" value="5" min="3" step="1">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label-custom">Side Length (s)</label>
                        <input type="number" id="in-length" class="form-control form-control-lg rounded-3" value="10" step="any">
                    </div>
                    <div class="col-md-4">
                ...
                <div class="mt-4 d-flex flex-wrap gap-2">
                    <button type="button" class="btn btn-primary btn-lg rounded-pill px-5 fw-bold shadow-sm" id="btn-calc"><i class="fas fa-calculator me-2"></i>Calculate Area</button>
        ...
        <div class="output-card-themed" style="--tool-hue:210;--tool-color:#2563eb;--tool-bg:rgba(59,130,246,.04);">
            ...
            <div class="row g-3 mt-4">
                <div class="col-6 col-md-6">
                    <div class="stat-card">
                        <span class="stat-card-label">Perimeter</span>
                        <span class="stat-card-value" id="out-perimeter">—</span>
                    </div>
                </div>
                <div class="col-6 col-md-6">
                    <div class="stat-card">
                        <span class="stat-card-label">Interior Angle</span>
                        <span class="stat-card-value" id="out-angle">—</span>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-5">
                ...
                <div class="p-4 bg-white rounded-4 border border-light-subtle shadow-sm">
                    <div class="step-item mb-4">
                        <div class="step-label">1. Formula for Regular Polygon Area</div>
                        <div class="step-formula">A = \frac{n \times s^2}{4 \times \tan(\pi/n)}</div>
                    </div>
                    <div class="step-item mb-4">
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function calculate(){
        const n = parseInt($('in-sides').value);
        const s = parseFloat($('in-length').value);
        ...
        const fPerim = perimeter.toFixed(prec);
        const fAngle = interiorAngle.toFixed(2);

        $('out-area').textContent = fArea;
        $('out-unit-display').textContent = `sq. ${unit}`;
        $('out-perimeter').textContent = `${fPerim} ${unit}`;
        $('out-angle').textContent = `${fAngle}°`;

        // Steps
        $('step-substitution').textContent = `A = (${n} \\times ${s}^2) / (4 \\times \\tan(180/${n})°)`;
    ...
    $('btn-copy').addEventListener('click', function(){
        const text = `Regular Polygon Area Calculation\n` +
                     `--------------------------\n` +
                     `Sides (n): ${$('in-sides').value}\n` +
                     `Side Length (s): ${$('in-length').value} ${$('in-unit').value}\n` +
                     `Perimeter: ${$('out-perimeter').textContent}\n` +
                     `Interior Angle: ${$('out-angle').textContent}\n` +
    ...
    ['input', 'change'].forEach(evt => {
        $('in-sides').addEventListener(evt, calculate);
        $('in-length').addEventListener(evt, calculate);
...
<?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\area-of-a-regular-polygon-calculator.blade.php ENDPATH**/ ?>