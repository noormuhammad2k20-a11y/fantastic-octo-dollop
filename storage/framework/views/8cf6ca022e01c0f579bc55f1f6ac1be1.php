<div class="interactive-tool-grid cap-rate-calculator">
    <div class="calculator-card">
        <div class="calculator-header">
            <div class="tool-icon-circle"><i class="fas fa-percentage"></i></div>
            <div><h4>Cap Rate Calculator</h4><p>Accurate Estimation Tool</p></div>
        </div>
        <div class="calculator-body">            <div class="form-group-custom mb-3"><label class="form-label-custom">Annual NOI ($)</label><input type="number" id="noi" class="form-control-custom" placeholder="e.g. 50000"></div>
            <div class="form-group-custom mb-3"><label class="form-label-custom">Property Price ($)</label><input type="number" id="prop-price" class="form-control-custom" placeholder="e.g. 800000"></div>
</div>
    </div>
    <div class="result-panel">
        <div class="result-card-v2">
            ...
            <div class="result-sub-stats">                <div class="stat-item border-end pe-3"><span class="stat-label">Cap Rate %</span><span class="stat-value" id="cap-res">0</span></div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    function calculate() {
        ...
        const prop_price = parseFloat(document.getElementById('prop-price').value) || 0;


        let res = 0; let mainUnit = '$';
        res = prop_price ? (noi / prop_price) * 100 : 0; mainUnit = '%'; try { document.getElementById('cap-res').innerText = res.toFixed(2)+'%'; } catch(e){}
    ...
    ['noi', 'prop-price'].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.addEventListener('input', calculate);
    });
    document.getElementById('copy-result').addEventListener('click', function() {
        const text = 'Cap Rate Result: ' + document.getElementById('main-result').innerText;
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\cap-rate-calculator.blade.php ENDPATH**/ ?>