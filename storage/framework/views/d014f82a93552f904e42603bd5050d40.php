    ...
    <div class="card border-0 shadow-sm p-4 mb-4 bg-white rounded-4">
        <div class="row g-3 align-items-end mb-4">
            <div class="col-md-4">
                ...
                <input type="text" id="bookTitle" class="form-control border-2" placeholder="e.g. Atomic Habits">
            </div>            <div class="col-md-4">
                <label class="form-label fw-bold small text-uppercase text-muted">Summary Style</label>
                <select id="summaryType" class="form-select border-2">
            ...
            <div class="col-md-4">
                <button id="gen-btn" class="btn btn-accent px-4 py-2 w-100 fw-bold rounded-3">
                    <i class="fas fa-book-open me-2"></i> Generate
                </button>
            </div>
        </div>
        <div id="gen-results" class="d-none">
            <h5 class="fw-bold mb-3"><i class="fas fa-book-open me-2 text-primary"></i>Book Summary</h5>
        ...
        <div id="gen-placeholder" class="text-center py-5">
            ...
            <h5 class="text-muted">Enter a book title to get a summary</h5>
        </div>
    </div>
</div>
<style>
.btn-accent{background:linear-gradient(135deg,#4facfe,#00f2fe);color:#fff;border:none;transition:.3s}
...
.list-group-item-action:hover{border-color:#4facfe!important;background:#fafafa}
.copy-icon{opacity:0;transition:.2s}
.list-group-item-action:hover .copy-icon{opacity:1}
</style>
<script>
document.addEventListener('DOMContentLoaded',function(){
const btn=document.getElementById('gen-btn'),bookTitle=document.getElementById('bookTitle'),summaryType=document.getElementById('summaryType'),results=document.getElementById('gen-results'),list=document.getElementById('gen-list'),ph=document.getElementById('gen-placeholder');
btn.addEventListener('click',function(){
    btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin me-2"></i>Generating...';
    fetch('<?php echo e(route("ai.generate",["type"=>"book-summarizer"])); ?>',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'},body:JSON.stringify({bookTitle:bookTitle.value,summaryType:summaryType.value})})
    .then(r=>r.json()).then(data=>{
        ...
        list.appendChild(d);});}
    }).finally(function(){btn.disabled=false;btn.innerHTML='<i class="fas fa-book-open me-2"></i>Generate';});
});
});
</script><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\book-summarizer.blade.php ENDPATH**/ ?>