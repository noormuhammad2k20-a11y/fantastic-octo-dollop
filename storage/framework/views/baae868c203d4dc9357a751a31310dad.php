        ...
        <div class="calculator-card">
            <div class="calculator-header d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center gap-2">
                    <div class="tool-icon-circle" style="background:rgba(234,88,12,.05);color:#ea580c"><i class="fas fa-weight-hanging"></i></div>
                    <h5 class="mb-0 fw-bold">Lumber Weight Calculator</h5>
                </div>
            ...
            <div class="calculator-body mt-2">
                <div class="row g-2">
                    <div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Species</label><select id='calc-species' class='form-select form-select-sm rounded-2 shadow-none border-secondary-subtle py-1'><option value='pine' >Pine</option><option value='oak' >Oak</option><option value='fir' >Fir</option></select></div><div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Size</label><select id='calc-nominal-size' class='form-select form-select-sm rounded-2 shadow-none border-secondary-subtle py-1'><option value='2x4' >2x4</option><option value='2x6' >2x6</option><option value='4x4' >4x4</option></select></div><div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Length (ft)</label><input type='number' id='calc-length' class='form-control form-control-sm rounded-2 shadow-none border-secondary-subtle py-1' value='8' step='1'></div><div class='col-6 col-md-4 mb-2'><label class='form-label-custom'>Moisture %</label><input type='number' id='calc-moisture' class='form-control form-control-sm rounded-2 shadow-none border-secondary-subtle py-1' value='15' step='1'></div>
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function renderFormula() {
        if (typeof katex !== 'undefined') {
            katex.render("Weight = Volume \\times Density", $('latex-formula'), {throwOnError: false, displayMode: true});
        } else setTimeout(renderFormula, 100);
    }
    renderFormula();

        function calculate() {
        const s = $('calc-species').value, len = parseFloat($('calc-length').value)||0;
        let w = (1.5*3.5*len*12/1728) * {pine:30, oak:45, fir:32}[s];
        $('res-main').textContent = Math.round(w);
        $('res-unit').textContent = 'Lbs';
        $('res-details').innerHTML = `<div class="stat-box"><div class="small text-muted">Species</div><div class="fw-bold text-uppercase">${s}</div></div>`;
    }
    $('btn-calc').addEventListener('click', calculate);
    ['calc-species','calc-length'].forEach(id=>$(id).addEventListener('change',calculate));
    calculate();
});
</script>
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\lumber-weight-calculator.blade.php ENDPATH**/ ?>