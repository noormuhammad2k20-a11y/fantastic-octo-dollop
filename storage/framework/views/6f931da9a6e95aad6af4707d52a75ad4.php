<div class="interactive-tool-grid roofing-cost-calculator">
    <div class="calculator-card">
        <div class="calculator-header">
            <div class="tool-icon-circle"><i class="fas fa-house-chimney"></i></div>
            <div><h4>Roofing Cost Calculator</h4><p>Accurate Estimation Tool</p></div>
        </div>
        <div class="calculator-body">            <div class="form-group-custom mb-3"><label class="form-label-custom">Area (SQFT)</label><input type="number" id="roof-area" class="form-control-custom" placeholder="e.g. 2000"></div>
            <div class="form-group-custom mb-3"><label class="form-label-custom">Material</label><select id="mat" class="form-select-custom">                        <option value="sh">Shingle</option>
    ...
    <div class="result-panel">
        <div class="result-card-v2">
            ...
            <div class="result-sub-stats">                <div class="stat-item border-end pe-3"><span class="stat-label">Est. Cost</span><span class="stat-value" id="roof-total">0</span></div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    function calculate() {
        const roof_area = parseFloat(document.getElementById('roof-area').value) || 0;
        const mat = document.getElementById('mat').value;


        let res = 0; let mainUnit = '$';
        res = roof_area * (mat === 'mt' ? 10 : 4); try { document.getElementById('roof-total').innerText = '$'+res.toLocaleString(); } catch(e){}
    ...
    ['roof-area', 'mat'].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.addEventListener('input', calculate);
    });
    document.getElementById('copy-result').addEventListener('click', function() {
        const text = 'Roofing Cost Result: ' + document.getElementById('main-result').innerText;
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\roofing-cost-calculator.blade.php ENDPATH**/ ?>