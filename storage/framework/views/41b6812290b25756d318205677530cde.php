<div class="interactive-tool-grid closing-cost-calculator">
    <div class="calculator-card">
        <div class="calculator-header">
            <div class="tool-icon-circle"><i class="fas fa-home"></i></div>
            <div><h4>Closing Cost Calculator</h4><p>Accurate Estimation Tool</p></div>
        </div>
        <div class="calculator-body">            <div class="form-group-custom mb-3"><label class="form-label-custom">Home Price ($)</label><input type="number" id="home-price" class="form-control-custom" placeholder="e.g. 300000"></div>
</div>
    </div>
    <div class="result-panel">
        <div class="result-card-v2">
            ...
            <div class="result-sub-stats">                <div class="stat-item border-end pe-3"><span class="stat-label">Legal Fees</span><span class="stat-value" id="legal-fees">0</span></div>
                <div class="stat-item ps-3"><span class="stat-label">Transfer Tax</span><span class="stat-value" id="transfer-tax">0</span></div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    function calculate() {
        const home_price = parseFloat(document.getElementById('home-price').value) || 0;


        let res = 0; let mainUnit = '$';
        res = home_price * 0.05; try { document.getElementById('legal-fees').innerText = '$' + (home_price * 0.01).toLocaleString(); document.getElementById('transfer-tax').innerText = '$' + (home_price * 0.02).toLocaleString(); } catch(e){}
    ...
    ['home-price'].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.addEventListener('input', calculate);
    });
    document.getElementById('copy-result').addEventListener('click', function() {
        const text = 'Closing Cost Result: ' + document.getElementById('main-result').innerText;
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\closing-cost-calculator.blade.php ENDPATH**/ ?>