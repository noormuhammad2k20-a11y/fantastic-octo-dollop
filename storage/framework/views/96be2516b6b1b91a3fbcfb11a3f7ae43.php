<div class="interactive-tool-grid emergency-fund-calculator">
    <div class="calculator-card">
        <div class="calculator-header">
            <div class="tool-icon-circle"><i class="fas fa-shield-alt"></i></div>
            <div><h4>Emergency Fund Calculator</h4><p>Accurate Estimation Tool</p></div>
        </div>
        <div class="calculator-body">            <div class="form-group-custom mb-3"><label class="form-label-custom">Monthly Expenses ($)</label><input type="number" id="monthly-cost" class="form-control-custom" placeholder="e.g. 3000"></div>
            <div class="form-group-custom mb-3"><label class="form-label-custom">Months Coverage</label><input type="number" id="months-cover" class="form-control-custom" placeholder="e.g. 6"></div>
</div>
    </div>
    <div class="result-panel">
        <div class="result-card-v2">
            ...
            <div class="result-sub-stats">                <div class="stat-item border-end pe-3"><span class="stat-label">Savings Goal</span><span class="stat-value" id="savings-goal">0</span></div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    function calculate() {
        const monthly_cost = parseFloat(document.getElementById('monthly-cost').value) || 0;
        const months_cover = parseFloat(document.getElementById('months-cover').value) || 0;


        let res = 0; let mainUnit = '$';
        res = monthly_cost * months_cover; try { document.getElementById('savings-goal').innerText = '$' + res.toLocaleString(); } catch(e){}
    ...
    ['monthly-cost', 'months-cover'].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.addEventListener('input', calculate);
    });
    document.getElementById('copy-result').addEventListener('click', function() {
        const text = 'Emergency Fund Result: ' + document.getElementById('main-result').innerText;
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\emergency-fund-calculator.blade.php ENDPATH**/ ?>