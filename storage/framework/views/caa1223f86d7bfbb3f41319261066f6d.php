<div class="interactive-tool-grid passive-income-calculator">
    <div class="calculator-card">
        <div class="calculator-header">
            <div class="tool-icon-circle"><i class="fas fa-couch"></i></div>
            <div><h4>Passive Income Calculator</h4><p>Accurate Estimation Tool</p></div>
        </div>
        <div class="calculator-body">            <div class="form-group-custom mb-3"><label class="form-label-custom">Capital ($)</label><input type="number" id="capital" class="form-control-custom" placeholder="e.g. 100000"></div>
            <div class="form-group-custom mb-3"><label class="form-label-custom">Yield (%)</label><input type="number" id="yield" class="form-control-custom" placeholder="e.g. 5"></div>
</div>
    </div>
    <div class="result-panel">
        <div class="result-card-v2">
            ...
            <div class="result-sub-stats">                <div class="stat-item border-end pe-3"><span class="stat-label">Monthly Inc</span><span class="stat-value" id="monthly-inc">0</span></div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    function calculate() {
        ...
        let dInc = (capital * (yield_val / 100)) / 12; res = dInc * 12; try { document.getElementById('monthly-inc').innerText = '$'+dInc.toLocaleString(); } catch(e){}
    ...
    ['capital', 'yield'].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.addEventListener('input', calculate);
    });
    document.getElementById('copy-result').addEventListener('click', function() {
        const text = 'Passive Income Result: ' + document.getElementById('main-result').innerText;
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\passive-income-calculator.blade.php ENDPATH**/ ?>