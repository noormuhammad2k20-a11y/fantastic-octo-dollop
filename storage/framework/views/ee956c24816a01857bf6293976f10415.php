        ...
        <div class="calculator-card">
            <div class="calculator-header d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center gap-2">
                    <div class="tool-icon-circle" style="background:rgba(234,88,12,.05);color:#ea580c"><i class="fas fa-truck-front"></i></div>
                    <h5 class="mb-0 fw-bold">Concrete Quantity Calculator</h5>
                </div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function renderFormula() {
        if (typeof katex !== 'undefined') {
            katex.render("Total Yards = \\frac{V_{cuft}}{27}", $('latex-formula'), {throwOnError: false, displayMode: true});
        } else setTimeout(renderFormula, 100);
    }
    renderFormula();

        function calculate() {
        const v = parseFloat($('calc-total-volume').value)||0, wast = parseFloat($('calc-wastage').value)||0;
        $('res-main').textContent = Math.ceil(v/0.6 * (1+wast/100));
        $('res-unit').textContent = 'Bags';
        $('res-details').innerHTML = `<div class="stat-box"><div class="small text-muted">Yards</div><div class="fw-bold">${(v/27).toFixed(2)}</div></div>`;
    }
    $('btn-calc').addEventListener('click', calculate);
    ['calc-total-volume','calc-wastage'].forEach(id=>$(id).addEventListener('input',calculate));
    calculate();
});
</script>
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\concrete-quantity-calculator.blade.php ENDPATH**/ ?>