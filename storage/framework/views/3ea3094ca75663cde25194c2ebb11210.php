<div class="interactive-tool-grid rental-yield-calculator">
    <div class="calculator-card">
        <div class="calculator-header">
            <div class="tool-icon-circle"><i class="fas fa-key"></i></div>
            <div><h4>Rental Yield Calculator</h4><p>Accurate Estimation Tool</p></div>
        </div>
        <div class="calculator-body">            <div class="form-group-custom mb-3"><label class="form-label-custom">Property Value ($)</label><input type="number" id="prop-val" class="form-control-custom" placeholder="e.g. 400000"></div>
            <div class="form-group-custom mb-3"><label class="form-label-custom">Monthly Rent ($)</label><input type="number" id="monthly-rent" class="form-control-custom" placeholder="e.g. 2000"></div>
</div>
    </div>
    <div class="result-panel">
        <div class="result-card-v2">
            ...
            <div class="result-sub-stats">                <div class="stat-item border-end pe-3"><span class="stat-label">Gross Yield %</span><span class="stat-value" id="gross-yield">0</span></div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    function calculate() {
        const prop_val = parseFloat(document.getElementById('prop-val').value) || 0;
        const monthly_rent = parseFloat(document.getElementById('monthly-rent').value) || 0;


        let res = 0; let mainUnit = '$';
        res = prop_val ? ((monthly_rent * 12) / prop_val) * 100 : 0; mainUnit = '%'; try { document.getElementById('gross-yield').innerText = res.toFixed(2)+'%'; } catch(e){}
    ...
    ['prop-val', 'monthly-rent'].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.addEventListener('input', calculate);
    });
    document.getElementById('copy-result').addEventListener('click', function() {
        const text = 'Rental Yield Result: ' + document.getElementById('main-result').innerText;
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\rental-yield-calculator.blade.php ENDPATH**/ ?>