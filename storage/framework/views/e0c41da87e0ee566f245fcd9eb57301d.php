        ...
        <div class="calculator-card">
            <div class="calculator-header d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center gap-2">
                    <div class="tool-icon-circle" style="background:rgba(234,88,12,.05);color:#ea580c"><i class="fas fa-square"></i></div>
                    <h5 class="mb-0 fw-bold">Squareness Checker</h5>
                </div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function renderFormula() {
        if (typeof katex !== 'undefined') {
            katex.render("c = \\sqrt{a^2 + b^2}", $('latex-formula'), {throwOnError: false, displayMode: true});
        } else setTimeout(renderFormula, 100);
    }
    renderFormula();

        function calculate() {
        const a = parseFloat($('calc-side-a').value)||0, b = parseFloat($('calc-side-b').value)||0, m = parseFloat($('calc-diag-meas').value)||0;
        const ideal = Math.sqrt(a*a+b*b);
        $('res-main').textContent = ideal.toFixed(2);
        $('res-unit').textContent = 'Ideal';
        $('res-details').innerHTML = `<div class="stat-box"><div class="small text-muted">Difference</div><div class="fw-bold">${Math.abs(m-ideal).toFixed(2)}</div></div>`;
    }
    $('btn-calc').addEventListener('click', calculate);
    ['calc-side-a','calc-side-b','calc-diag-meas'].forEach(id=>$(id).addEventListener('input',calculate));
    calculate();
});
</script>
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\squareness-checker.blade.php ENDPATH**/ ?>