<div class="interactive-tool-grid loan-eligibility-calculator">
    <div class="calculator-card">
        <div class="calculator-header">
            <div class="tool-icon-circle"><i class="fas fa-user-check"></i></div>
            <div><h4>Loan Eligibility Calculator</h4><p>Accurate Estimation Tool</p></div>
        </div>
        <div class="calculator-body">            <div class="form-group-custom mb-3"><label class="form-label-custom">Monthly Inc ($)</label><input type="number" id="income-val" class="form-control-custom" placeholder="e.g. 5000"></div>
            <div class="form-group-custom mb-3"><label class="form-label-custom">Existing Debt ($)</label><input type="number" id="debts" class="form-control-custom" placeholder="e.g. 500"></div>
</div>
    </div>
    <div class="result-panel">
        <div class="result-card-v2">
            ...
            <div class="result-sub-stats">                <div class="stat-item border-end pe-3"><span class="stat-label">Max Loan</span><span class="stat-value" id="max-loan">0</span></div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    function calculate() {
        const income_val = parseFloat(document.getElementById('income-val').value) || 0;
        const debts = parseFloat(document.getElementById('debts').value) || 0;


        let res = 0; let mainUnit = '$';
        res = Math.max(0, ((income_val * 0.43) - debts) * 100); try { document.getElementById('max-loan').innerText = '$'+res.toLocaleString(); } catch(e){}
    ...
    ['income-val', 'debts'].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.addEventListener('input', calculate);
    });
    document.getElementById('copy-result').addEventListener('click', function() {
        const text = 'Loan Eligibility Result: ' + document.getElementById('main-result').innerText;
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\loan-eligibility-calculator.blade.php ENDPATH**/ ?>