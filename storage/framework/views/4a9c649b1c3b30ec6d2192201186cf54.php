        ...
        <div class="calculator-card">
            <div class="calculator-header d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center gap-2">
                    <div class="tool-icon-circle" style="background:rgba(234,88,12,.05);color:#ea580c"><i class="fas fa-border-all"></i></div>
                    <h5 class="mb-0 fw-bold">Retaining Wall Calculator</h5>
                </div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function renderFormula() {
        if (typeof katex !== 'undefined') {
            katex.render("Blocks = \\frac{L \\times H}{Block Area} \\times (1 + W)", $('latex-formula'), {throwOnError: false, displayMode: true});
        } else setTimeout(renderFormula, 100);
    }
    renderFormula();

        function calculate() {
        const l = parseFloat($('calc-wall-len').value)||0, h = parseFloat($('calc-wall-height').value)||0, wast = parseFloat($('calc-wastage').value)||0;
        let n = Math.ceil((l*h)/0.5 * (1+wast/100));
        $('res-main').textContent = n.toLocaleString();
        $('res-unit').textContent = 'Blocks';
        $('res-details').innerHTML = `<div class="stat-box"><div class="small text-muted">Area</div><div class="fw-bold">${l*h} ft²</div></div>`;
    }
    $('btn-calc').addEventListener('click', calculate);
    ['calc-wall-len','calc-wall-height','calc-wastage'].forEach(id=>$(id).addEventListener('input',calculate));
    calculate();
});
</script>
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\retaining-wall-calculator.blade.php ENDPATH**/ ?>