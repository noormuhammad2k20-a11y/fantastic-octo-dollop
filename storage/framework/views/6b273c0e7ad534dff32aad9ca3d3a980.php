        ...
        <div class="calculator-card">
            <div class="calculator-header d-flex align-items-center justify-content-between">
                <div class="d-flex align-items-center gap-2">
                    <div class="tool-icon-circle" style="background:rgba(234,88,12,.05);color:#ea580c"><i class="fas fa-ruler-combined"></i></div>
                    <h5 class="mb-0 fw-bold">Stud Wall Framing Calculator</h5>
                </div>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    function renderFormula() {
        if (typeof katex !== 'undefined') {
            katex.render("Studs = \\left(\\frac{L}{OC} + 1\\right) + Extra", $('latex-formula'), {throwOnError: false, displayMode: true});
        } else setTimeout(renderFormula, 100);
    }
    renderFormula();

        function calculate() {
        const l = parseFloat($('calc-wall-len').value)||0, oc = parseInt($('calc-stud-spacing').value)||16;
        let n = Math.ceil((l*12)/oc)+1;
        $('res-main').textContent = n;
        $('res-unit').textContent = 'Studs';
        $('res-details').innerHTML = `<div class="stat-box"><div class="small text-muted">Spacing</div><div class="fw-bold">${oc}" OC</div></div>`;
    }
    $('btn-calc').addEventListener('click', calculate);
    ['calc-wall-len','calc-stud-spacing'].forEach(id=>$(id).addEventListener('input',calculate));
    calculate();
});
</script>
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\stud-wall-framing-calculator.blade.php ENDPATH**/ ?>