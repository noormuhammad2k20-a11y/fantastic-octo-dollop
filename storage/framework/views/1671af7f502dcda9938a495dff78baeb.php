            ...
            <div class="row g-4">
                ...
                <div class="col-md-6 text-center">
                    <div class="p-4 rounded-4 bg-light border h-100 d-flex flex-column justify-content-center">
                        <p class="text-muted fw-bold text-uppercase small letter-spacing-1 mb-2">Total Surface Area (A)</p>
                ...
                <div class="col-md-6 text-center">
                    <div class="p-4 rounded-4 bg-light border h-100 d-flex flex-column justify-content-center">
                        <p class="text-muted fw-bold text-uppercase small letter-spacing-1 mb-2">Lateral Area (L)</p>
                        <div class="display-5 fw-bold text-dark mb-0 text-break overflow-x-auto" id="out-lat">0.00</div>
                        <div class="mt-2 text-muted fw-bold unit-display" id="out-lat-unit"></div>
                    </div>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculate() {
      ...
      const lat = Math.PI * r * l;
      const area = Math.PI * r * (r + l);
      document.getElementById('out-area').textContent = area.toFixed(prec);
      document.getElementById('out-lat').textContent = lat.toFixed(prec);
    

        // Apply units
        const outputConfigs = [{"id":"out-area","label":"Total Surface Area (A)","suffixUnit":"sq units"},{"id":"out-lat","label":"Lateral Area (L)","suffixUnit":"sq units"}];
    ...
    btnCopyForm.addEventListener('click', function() {
        const formula = `$$L = \pi r \sqrt{h^2 + r^2}$$, $$A = \pi r(r + \sqrt{h^2 + r^2})$$`;
    ...
    btnCopyRes.addEventListener('click', function() {
        const txt = `Total Surface Area (A): ${document.getElementById('out-area').textContent} ${document.getElementById('out-area-unit').textContent} | Lateral Area (L): ${document.getElementById('out-lat').textContent} ${document.getElementById('out-lat-unit').textContent}`;
...
<?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views/tools/interactive/surface-area-of-a-cone-calculator.blade.php ENDPATH**/ ?>