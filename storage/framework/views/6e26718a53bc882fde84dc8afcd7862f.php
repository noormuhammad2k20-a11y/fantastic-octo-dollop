<div class="interactive-tool-grid financial-freedom-calculator">
    <div class="calculator-card">
        <div class="calculator-header">
            <div class="tool-icon-circle"><i class="fas fa-sun"></i></div>
            <div><h4>Financial Freedom Calculator</h4><p>Accurate Estimation Tool</p></div>
        </div>
        <div class="calculator-body">            <div class="form-group-custom mb-3"><label class="form-label-custom">Annual Expenses ($)</label><input type="number" id="annual-exp" class="form-control-custom" placeholder="e.g. 40000"></div>
            <div class="form-group-custom mb-3"><label class="form-label-custom">Safe Withdrawal (%)</label><input type="number" id="safe-w-rate" class="form-control-custom" placeholder="e.g. 4"></div>
</div>
    </div>
    <div class="result-panel">
        <div class="result-card-v2">
            ...
            <div class="result-sub-stats">                <div class="stat-item border-end pe-3"><span class="stat-label">FIRE Number</span><span class="stat-value" id="fire-number">0</span></div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    function calculate() {
        const annual_exp = parseFloat(document.getElementById('annual-exp').value) || 0;
        const safe_w_rate = parseFloat(document.getElementById('safe-w-rate').value) || 0;


        let res = 0; let mainUnit = '$';
        res = safe_w_rate ? (annual_exp / (safe_w_rate / 100)) : 0; try { document.getElementById('fire-number').innerText = '$' + res.toLocaleString(); } catch(e){}
    ...
    ['annual-exp', 'safe-w-rate'].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.addEventListener('input', calculate);
    });
    document.getElementById('copy-result').addEventListener('click', function() {
        const text = 'Financial Freedom Result: ' + document.getElementById('main-result').innerText;
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\financial-freedom-calculator.blade.php ENDPATH**/ ?>