<div class="interactive-tool-grid inheritance-tax-calculator">
    <div class="calculator-card">
        <div class="calculator-header">
            <div class="tool-icon-circle"><i class="fas fa-vault"></i></div>
            <div><h4>Inheritance Tax Calculator</h4><p>Accurate Estimation Tool</p></div>
        </div>
        <div class="calculator-body">            <div class="form-group-custom mb-3"><label class="form-label-custom">Legacy Amount ($)</label><input type="number" id="legacy-amt" class="form-control-custom" placeholder="e.g. 500000"></div>
</div>
    </div>
    <div class="result-panel">
        <div class="result-card-v2">
            ...
            <div class="result-sub-stats">                <div class="stat-item border-end pe-3"><span class="stat-label">Tax Due</span><span class="stat-value" id="inherit-tax">0</span></div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    function calculate() {
        const legacy_amt = parseFloat(document.getElementById('legacy-amt').value) || 0;


        let res = 0; let mainUnit = '$';
        res = legacy_amt * 0.15; try { document.getElementById('inherit-tax').innerText = '$'+res.toLocaleString(); } catch(e){}
    ...
    ['legacy-amt'].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.addEventListener('input', calculate);
    });
    document.getElementById('copy-result').addEventListener('click', function() {
        const text = 'Inheritance Tax Result: ' + document.getElementById('main-result').innerText;
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\inheritance-tax-calculator.blade.php ENDPATH**/ ?>