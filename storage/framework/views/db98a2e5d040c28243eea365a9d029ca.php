<div class="interactive-tool-grid cybersecurity-roi-calculator">
    <div class="calculator-card">
        <div class="calculator-header">
            <div class="tool-icon-circle"><i class="fas fa-shield-virus"></i></div>
            <div><h4>Cybersecurity ROI Calculator</h4><p>Accurate Estimation Tool</p></div>
        </div>
        <div class="calculator-body">            <div class="form-group-custom mb-3"><label class="form-label-custom">Prob (%)</label><input type="number" id="prob" class="form-control-custom" placeholder="e.g. 20"></div>
            <div class="form-group-custom mb-3"><label class="form-label-custom">Impact ($)</label><input type="number" id="imp" class="form-control-custom" placeholder="e.g. 1000000"></div>
            <div class="form-group-custom mb-3"><label class="form-label-custom">Spend ($)</label><input type="number" id="spend" class="form-control-custom" placeholder="e.g. 50000"></div>
</div>
    </div>
    <div class="result-panel">
        <div class="result-card-v2">
            ...
            <div class="result-sub-stats">                <div class="stat-item border-end pe-3"><span class="stat-label">ROI %</span><span class="stat-value" id="roi-val">0</span></div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    function calculate() {
        const prob = parseFloat(document.getElementById('prob').value) || 0;
        ...
        let ale = (prob/100)*imp; res = spend ? ((ale-spend)/spend)*100 : 0; mainUnit = '%'; try { document.getElementById('roi-val').innerText = res.toFixed(2)+'%'; } catch(e){}
    ...
    ['prob', 'imp', 'spend'].forEach(id => {
        const el = document.getElementById(id);
        if(el) el.addEventListener('input', calculate);
    });
    document.getElementById('copy-result').addEventListener('click', function() {
        const text = 'Cybersecurity ROI Result: ' + document.getElementById('main-result').innerText;
...
</style><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\cybersecurity-roi-calculator.blade.php ENDPATH**/ ?>