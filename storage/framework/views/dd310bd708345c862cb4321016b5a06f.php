    ...
    <div class="card tool-card-stacked mb-4 shadow-sm border-0">
        <div class="card-header-v2 bg-white border-bottom-0 py-4 px-4">
            <div class="d-flex align-items-center">
                ...
                <div>
                    <h1 class="mb-0 fw-bold text-dark fs-5">Surface Area of a Triangular Prism Calculator</h1>
                    <p class="text-muted small mb-0">Calculate the surface area of a triangular prism</p>
                </div>
            </div>
        </div>
        <div class="card-body-v2 px-4 pb-4">
            <div class="row g-4">
                ...
                <div class="col-md-6 col-lg-4">
                    ...
                    <input type="number" id="in-b" class="form-control form-control-lg rounded-3" value="0" step="any" min="0">
                </div>
                <div class="col-md-6 col-lg-4">
                    <label class="form-label small fw-bold text-secondary text-uppercase mb-2">Triangle Height (h)</label>
                    <input type="number" id="in-h" class="form-control form-control-lg rounded-3" value="0" step="any" min="0">
                </div>
                <div class="col-md-6 col-lg-4">
                    <label class="form-label small fw-bold text-secondary text-uppercase mb-2">Side 1 (s₁)</label>
                    <input type="number" id="in-s1" class="form-control form-control-lg rounded-3" value="0" step="any" min="0">
                </div>
                <div class="col-md-6 col-lg-4">
                    <label class="form-label small fw-bold text-secondary text-uppercase mb-2">Side 2 (s₂)</label>
                    <input type="number" id="in-s2" class="form-control form-control-lg rounded-3" value="0" step="any" min="0">
                </div>
                <div class="col-md-6 col-lg-4">
                    <label class="form-label small fw-bold text-secondary text-uppercase mb-2">Prism Length (l)</label>
                    <input type="number" id="in-l" class="form-control form-control-lg rounded-3" value="0" step="any" min="0">
                </div>
                <div class="col-md-6 col-lg-4">
...
<option value="mm">mm</option><option value="cm">cm</option><option value="m">m</option><option value="km">km</option><option value="in">in</option><option value="ft">ft</option><option value="yd">yd</option><option value="mi">mi</option>
            ...
            <div class="quick-actions-bar mt-4">
                ...
                <button type="button" class="qa-btn qa-copy-formula" id="btn-copy-formula"><i class="fas fa-superscript me-2"></i>Copy Formula</button>
            </div>
        </div>
    </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-md-12 text-center">
                    <div class="p-4 rounded-4 bg-light border h-100 d-flex flex-column justify-content-center">
                        <p class="text-muted fw-bold text-uppercase small letter-spacing-1 mb-2">Surface Area (A)</p>
                        <div class="display-5 fw-bold text-dark mb-0 text-break overflow-x-auto" id="out-area">0.00</div>
                        <div class="mt-2 text-muted fw-bold unit-display" id="out-area-unit"></div>
                    </div>
                </div>
            </div>
            
            <div class="mt-4 p-4 rounded-4 bg-white border shadow-sm text-center overflow-x-auto" id="formula-container">
                <h6 class="fw-bold mb-3 small text-uppercase text-muted letter-spacing-1">Mathematical Formula</h6>
<div class="fs-5 mb-2">$$A = bh + (b + s_1 + s_2)l$$</div>
            </div>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculate() {
      ...
      const s1 = parseFloat(document.getElementById('in-s1').value);
      const s2 = parseFloat(document.getElementById('in-s2').value);
      const l = parseFloat(document.getElementById('in-l').value);
      if(isNaN(b) || isNaN(h) || isNaN(s1) || isNaN(s2) || isNaN(l) || b<0 || h<0 || s1<0 || s2<0 || l<0) return;
      const area = (b * h) + ((b + s1 + s2) * l);
      document.getElementById('out-area').textContent = area.toFixed(prec);
    

        // Apply units
        const outputConfigs = [{"id":"out-area","label":"Surface Area (A)","suffixUnit":"sq units"}];
    ...
    btnReset.addEventListener('click', () => {
        ...
        document.getElementById('in-s1').value = 0;
        document.getElementById('in-s2').value = 0;
    ...
    btnCopyForm.addEventListener('click', function() {
        const formula = `$$A = bh + (b + s_1 + s_2)l$$`;
    ...
    btnCopyRes.addEventListener('click', function() {
        const txt = `Surface Area (A): ${document.getElementById('out-area').textContent} ${document.getElementById('out-area-unit').textContent}`;
...
<?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views/tools/interactive/surface-area-of-a-triangular-prism-calculator.blade.php ENDPATH**/ ?>