        ...
        <div class="row g-3 align-items-end mb-4">
            ...
            <div class="col-md-4">
                <button id="gen-btn" class="btn btn-accent px-4 py-2 w-100 fw-bold rounded-3">
                    <i class="fas fa-guitar me-2"></i> Generate
                </button>
            </div>
        </div>
        <div id="gen-results" class="d-none">
            <h5 class="fw-bold mb-3"><i class="fas fa-guitar me-2 text-danger"></i>Generated Band Names</h5>
            <div id="gen-list" class="list-group gap-2"></div>
        </div>
        <div id="gen-placeholder" class="text-center py-5">
            <div class="opacity-25 mb-3"><i class="fas fa-guitar fa-4x"></i></div>
            <h5 class="text-muted">Choose a genre to generate band names</h5>
        </div>
    </div>
</div>
<style>
.btn-accent{background:linear-gradient(135deg,#f12711,#f5af19);color:#fff;border:none;transition:.3s}
...
.list-group-item-action:hover{border-color:#f12711!important;background:#fafafa}
.copy-icon{opacity:0;transition:.2s}
.list-group-item-action:hover .copy-icon{opacity:1}
</style>
<script>
document.addEventListener('DOMContentLoaded',function(){
const btn=document.getElementById('gen-btn'),genre=document.getElementById('genre'),vibe=document.getElementById('vibe'),results=document.getElementById('gen-results'),list=document.getElementById('gen-list'),ph=document.getElementById('gen-placeholder');
btn.addEventListener('click',function(){
    btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin me-2"></i>Generating...';
    fetch('<?php echo e(route("ai.generate",["type"=>"band-name"])); ?>',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-TOKEN':'<?php echo e(csrf_token()); ?>'},body:JSON.stringify({genre:genre.value,vibe:vibe.value})})
    .then(r=>r.json()).then(data=>{
        ...
        list.appendChild(d);});}
    }).finally(function(){btn.disabled=false;btn.innerHTML='<i class="fas fa-guitar me-2"></i>Generate';});
});
});
</script><?php /**PATH D:\Xamp\htdocs\ToolsHub\resources\views\tools\interactive\ai-band-name-generator.blade.php ENDPATH**/ ?>