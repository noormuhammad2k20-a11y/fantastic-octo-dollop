...
// Find the line with "Global initialization" or "NEW BATCH: FINANCIAL CALCULATORS (SEO-FIRST PLATFORM)"
$cutIndex = -1;
for ($i = 0; $i < count($lines); $i++) {
    if (strpos($lines[$i], 'Global initialization') !== false) {
        $cutIndex = $i;
        break;
    }
}
...
if ($cutIndex !== -1) {
    // Keep only the class up to its end brace
    $newLines = array_slice($lines, 0, $cutIndex);
    // Trim back to the end of class brace
    while (!empty($newLines) && trim(end($newLines)) !== "}") {
        array_pop($newLines);
    }
    
    // pop the final "}" to append methods inside class
    $lastLine = array_pop($newLines);
    if (trim($lastLine) !== "}") {
       array_push($newLines, $lastLine); 
    }

    $baseJs = implode("", $newLines);
...
JAVASCRIPT;
    ...
    $finalJs = $baseJs . $injection;
    file_put_contents($jsPath, $finalJs);
    echo "Fixed syntax and logic appended successfully!\n";
} else {
    echo "Could not parse correctly.\n";
}
