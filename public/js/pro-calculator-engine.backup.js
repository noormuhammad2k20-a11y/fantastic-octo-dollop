...
RewriteRule \.(jpg|jpeg|png|gif)$ - [F,NC,L]`.trim();
    ...
    /* 40 ── Sourdough Hydration ─────────────────────────────── */
    sourdough_calc(s) {
        const flour = parseFloat(s.flour) || 0;
        const water = parseFloat(s.water) || 0;
        const starter = parseFloat(s.starter_amount) || 0;
        const sHydration = parseFloat(s.starter_hydration) || 100;
        
        const sFlour = starter / (1 + (sHydration / 100));
        const sWater = starter - sFlour;
        
        const totalFlour = flour + sFlour;
        const totalWater = water + sWater;
        const hydration = (totalWater / totalFlour) * 100;

        return {
            mainValue: hydration.toFixed(1) + '%',
            mainLabel: 'Total Hydration',
            subStats: [
                { label: 'Total Flour', value: Math.round(totalFlour) + 'g' },
                { label: 'Total Water', value: Math.round(totalWater) + 'g' }
            ],
            insights: [
                `High hydration (75%+) results in an <strong>open, airy crumb</strong>.`,
                `Low hydration (<65%) makes dough easier to shape and score.`,
                `Calculated by factoring in the <strong>water/flour ratio</strong> of the starter.`
            ]
        };
    }
